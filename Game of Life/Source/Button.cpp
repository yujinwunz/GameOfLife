...
void Button::draw(SDL_Surface* s){
	if(enabled){
		...
		//draw boring old disabled box
		SDL_FillRect(s,NULL,(backColour.r<<16)|(backColour.g<<8)|backColour.b);
	}
	
	//draw text
	if(hovered&&leftDown && enabled){
		drawText(text,width/2+_bBT,height/2+_bBT,font,s,textColour.r,textColour.g,
